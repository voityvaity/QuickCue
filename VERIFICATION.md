...
Подтверждено локально для этапов 07–11:
...
- `git diff --check`: PASS.
- Добавлены regression-тесты prompt/context revision, управляемого photo route/аппаратных фаз, speech benchmark/timing, V3→V4→V5→V6→V7, ручного приоритета ролей, stale/неуверенных A/B-меток и очистки in-memory аудиобуфера.

Не выполнены на Windows:

- Xcode typecheck, iOS XCTest, Simulator/iphoneos build и проверка source revision внутри новой IPA.
- Миграция реальной установленной базы через V4 → V7, UI камеры/импорта/OCR, физическая кнопка, 50-фразовый Speech-прогон и сохранение старых данных непосредственно на iPhone.
...
Обновление устанавливать поверх прежнего с тем же bundle ID и совместимой подписью. Старую установку не удалять. Автоматической телеметрии, новых ключей, облачной диаризации и платных ПК-проверок в этом проходе нет. Этап 12 не начинался.
